...
The app has a fourth "Admin" tab, password-gated with `ADMIN_PASSWORD`. Once
unlocked it shows every uploaded photo in a grid with a delete button on each
— handy if someone uploads the wrong photo — plus the "Clear all photos"
button that used to live under Slideshow. The password is verified against
the real server (not just hidden in the page), so it's actual access control,
not just obscurity.
...
- `POST /api/admin/verify` — checks `x-admin-password` header, no side effects
